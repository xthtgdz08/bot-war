$)C//////////////////////////////////////////////////////
//========= Yu cu tt c cc bin cn s dng =========//
/////////////////////////////////////////////////////
...
const logger = require("./core/script/log.js");
...
try {
    ...
    else return logger.loader("config.json u Mt Ri Bro=))?", "error");
}

try {
    for (const key in configValue) global.config[key] = configValue[key];
    logger.loader("Config Done thnh cng!");
}
...
console.log(chalk.bold.hex("#FFFF00").bold("[ bot-war ]"));

////////////////////////////////////////////////////////////
//== ng nhp ti khon v bt u lng nghe s kin ====//
////////////////////////////////////////////////////////////
function onBot({ models }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        logger(`${startMem.ram.used}MB/${startMem.ram.total}MB  HEAP: ${startMem.heap.used}MB/${startMem.heap.total}MB`, "[ GIM ST ]");
        setInterval(() => {
            const currentMem = getMemoryInfo();
            logger(`${currentMem.ram.used}MB/${currentMem.ram.total}MB  HEAP: ${currentMem.heap.used}MB/${currentMem.heap.total}MB`, "[ GIM ST ]");
}, 30000);
      global.client.api = loginApiData
                    ...
                    try {
                        ...
                        if (module.config.dependencies && typeof module.config.dependencies == 'object') {
                            for (const reqDependencies in module.config.dependencies) {
                                const reqDependenciesPath = join(__dirname, 'nodemodules', 'node_modules', reqDependencies$)C);
                                try {
                    ...
                    try {
                        ...
                        if (event.config.envConfig) try {
                            for (const _0x5beea0 in event.config.envConfig) {
                                ...
                                if (typeof global.config$)C[event.config.name] == 'undefined') global.config[event.config.name] = {};
       ...
       // logger.loader('=== ' + (Date.now() - global.client.timeStart) + 'ms ===')
        ...
        const listener = require('./core/bot')(listenerData);
                 async function refreshFb_dtsg() {
        try {
            await api.refreshFb_dtsg();
            console.log('Reset fb_dtsg thnh cng!');
        } catch (err) {
            logger("error", " xy ra li khi lm mi fb_dtsg v jazoest", error);
        }
    }          
          ...
          if (error) {
            if (JSON.stringify(error).includes("601051028565049")) {
              ...
              api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
                ...
                if (e || res.errors) return logger("Li khng th xa cnh co ca facebook.", "error");
                if (res.data.fb_scraping_warning_clear.success) {
                  logger(" vt cnh co facebook thnh cng.", "[ SUCCESS ] >");
...
(async() => {
    try {
        ...
        authentication.sequelize = sequelize;
        const models = require('./core/database/model')(authentication);
